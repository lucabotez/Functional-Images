...
toTransformation (Combine transformations) = foldr (.) id (map toTransformation transformations)
...
toRegion (FromPoints point) = S.fromPoints point
toRegion (Rectangle width height) = S.rectangle width height
toRegion (Circle radius) = S.circle radius
toRegion (Complement regionAST) = S.complement (toRegion regionAST)
toRegion (Union region1AST region2AST) = S.union (toRegion region1AST) (toRegion region2AST)
toRegion (Intersection region1AST region2AST) = S.intersection (toRegion region1AST) (toRegion region2AST)
toRegion (Transform transformationAST regionAST) = S.applyTransformation (toTransformation transformationAST) (toRegion regionAST)
...
decomposeTransformation (Combine transformations) = concat (map decomposeTransformation transformations)
decomposeTransformation (transform) = [transform]
...
fuseTransformations transformations =
    let fuseAdj (Translation tx1 ty1) (Translation tx2 ty2 : other) =
            Translation (tx1 + tx2) (ty1 + ty2) : other
        fuseAdj (Scaling factor1) (Scaling factor2 : other) =
            Scaling (factor1 * factor2) : other
        fuseAdj transformation acc = transformation : acc
    in foldr fuseAdj [] transformations
...
-- creates a list of all the transformations (Combine)
modifyTransformations :: TransformationAST -> TransformationAST
modifyTransformations (Combine transforms) = combineTransformations $ map modifyTransformations $ decomposeTransformation $ Combine transforms
modifyTransformations other = other

-- fuses a given Combine transformation
fuseCombined :: TransformationAST -> TransformationAST
fuseCombined transforms = combineTransformations $ fuseTransformations $ decomposeTransformation transforms

-- appends all the transformations
concatTransformations :: TransformationAST -> TransformationAST -> TransformationAST
concatTransformations transforms1 transforms2 = combineTransformations $ decomposeTransformation transforms1 ++ decomposeTransformation transforms2

-- fuses two sets of transformations
optimizeTwoTransformations :: TransformationAST -> TransformationAST -> TransformationAST
optimizeTwoTransformations transforms1 transforms2 = (fuseCombined (concatTransformations (modifyTransformations transforms1) (modifyTransformations transforms2)))

optimizeTransformations :: RegionAST -> RegionAST
optimizeTransformations region = case region of
    Transform t1 (Transform t2 regionAST) ->
        optimizeTransformations $ applyTransformation (optimizeTwoTransformations t1 t2) regionAST
    Transform t1 regionAST -> case r of
        Transform t2 reg -> optimizeTransformations $ applyTransformation (optimizeTwoTransformations t1 t2) reg
        otherwise -> applyTransformation (fuseCombined t1) (optimizeTransformations regionAST)
        where r = optimizeTransformations regionAST
    Complement (Transform t regionAST) ->
        optimizeTransformations $ applyTransformation (fuseCombined t) (optimizeTransformations (Complement regionAST))
    Union region1 region2 ->
        (Union (optimizeTransformations region1) (optimizeTransformations region2))
    Intersection region1 region2 ->
        (Intersection (optimizeTransformations region1) (optimizeTransformations region2))
    otherwise ->
        region
