...
rectangle width height = \point ->
    let x = fst point
        y = snd point
    in x >= -width / 2.0 && x <= width / 2.0 && y <= height / 2.0 && y >= -height / 2.0
...
splitList :: Int -> [Char] -> [[Char]]
splitList _ [] = []
splitList n xs = take n xs : splitList n (drop n xs) 

plot :: Int -> Int -> Region -> String
plot width height region =
    let row y = [if inside (fromIntegral x, fromIntegral y) region then '*' else '.' | x <- [(-width)..width]]
        diagram = [row y | y <- [height, (height - 1)..(-height)]]
    in
        intercalate "\n" diagram
...
scaling factor = \point ->
    let x = fst point
        y = snd point
    in (x / factor, y / factor)
