...
indentList :: Int -> String
indentList level = replicate level ' '

showLevel :: Int -> RegionAST -> String
showLevel level (R (FromPoints points)) = indentList level ++ "FromPoints " ++ show points
showLevel level (R (Circle radius)) = indentList level ++ "Circle " ++ show radius
showLevel level (R (Rectangle width height)) = indentList level ++ "Rectangle " ++ show width ++ " " ++ show height
showLevel level (R (Complement region)) = indentList level ++ "~\n" ++ showLevel (level + 2) region
showLevel level (R (Union region1 region2)) = indentList level ++ "+\n" ++ showLevel (level + 2) region1 ++ "\n" ++ showLevel (level + 2) region2
showLevel level (R (Intersection region1 region2)) = indentList level ++ "*\n" ++ showLevel (level + 2) region1 ++ "\n" ++ showLevel (level + 2) region2
showLevel level (R (Transform transformation region)) = indentList level ++ show transformation ++ "\n" ++ showLevel (level + 2) region

instance Show RegionAST where
    show region = showLevel 0 region
...
instance Functor RegionShape where
    ...
    fmap f (Transform transformation a) = Transform transformation $ f a
...
basicTransformationCount = foldTransformationAST combiner
  where
    ...
    combiner (Combine transformations) = sum transformations
    combiner _ = 1
...
{-
    *** TODO ***
    ...
    Răspuns: se va evalua doar componenta accesata la momentul respectiv, nu intreaga pereche
...
-- basicRegionCount :: RegionAST -> Int
-- basicRegionCount (Union region1 region2) = basicRegionCount region1 + basicRegionCount region2
-- basicRegionCount (Intersection region1 region2) = basicRegionCount region1 + basicRegionCount region2
-- basicRegionCount (Complement region) = basicRegionCount region
-- basicRegionCount other = 1

basicEntityCount :: RegionAST -> (Int, Int)
basicEntityCount = foldRegionAST combiner
  where
    combiner :: RegionCombiner (Int, Int)
    combiner (FromPoints points) = (1, 0)
    combiner (Rectangle _ _) = (1, 0)
    combiner (Circle _) = (1, 0)
    combiner (Complement region) = region
    combiner (Transform transformation region) = (fst region, snd region + basicTransformationCount transformation)
    combiner (Union region1 region2) = (fst region1 + fst region2, snd region1 + snd region2)
    combiner (Intersection region1 region2) = (fst region1 + fst region2, snd region1 + snd region2)
